...
#define GAME_CLEAR 15 /*ゲームクリア条件(秒数)*/
    ...
    while(true){
        ...
        if(ch != ERR){
            ...
            case '1':
                ...
                mvprintw(1,1,"Health: %d |Score: %5d |Name: %4s |Time: %2.1f |",health,score,name,cpu_time_used); /*状態の表示*/
                for (l = 1; l < x-1; l++) {
                    mvaddch(2, l, '-');
                }
                ...
                while(health>0&&end!=true){
                    ...
                    end_t = time(NULL);
                    cpu_time_used = difftime(end_t,start_t);
                    if(cpu_time_used-1 >= GAME_CLEAR){
                        end = true;
                    }
                }
                if(health==0){
